...
Acceptance (cập nhật): file office là private signed URL nên KHÔNG dùng Google/Microsoft Docs viewer (phơi file cho bên thứ ba — vi phạm rule privacy, xem ADR-026). Preview client-side qua `officePreviewKind()` (`lib/resource/view.ts`): `.docx` → `docx-preview`, `.xlsx/.xls` → SheetJS (`components/viewer/xlsx-viewer.tsx`, bảng React tối đa 300 hàng × 40 cột/trang tính, chuyển sheet bằng tab), `.pptx` → `pptx-preview` (`components/viewer/pptx-viewer.tsx`). Tất cả lazy chunk, fetch signed URL về trình duyệt rồi render — bytes không rời khỏi user; áp dụng cả trang public/chia sẻ vì dùng chung `ResourceViewer`. Mọi loại office khi `ready` + có storage được ký URL đọc → nút "Tải về" hoạt động. `.doc/.ppt` cũ và lỗi render → fallback trung thực. Processing status n/a. Presentation mode/slide navigation để dành.
